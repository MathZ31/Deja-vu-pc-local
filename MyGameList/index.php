...
<body>
  ...
  // Traitement de la suppression d'un jeux-videos
  if (isset($_GET["action"]) && $_GET["action"] == "delete" && isset($_GET["id"])) {
    $requete = "DELETE FROM jeux_videos WHERE id = :id";
    ...
    if ($stmt->execute()) {
      ...
      <div class="alert alert-success alert-dismissible fade show" role="alert">
        Le jeux vidéos a bien été supprimé
  ...
  // Traitement de l'ajout ou de la modification d'un jeux videos
  if (isset($_POST['submit_game'])) {
    ...
    if (empty($errors)) {
      if (empty($id)) {
        // Insertion d'un nouveau jeux videos
        $requete = "INSERT INTO jeux_videos (image_url, titre, score, plateforme, date_debut, date_fin, temps_jeu, platine, commentaire, statut) 
                  VALUES (:image_url, :titre, :score, :plateforme, :date_debut, :date_fin, :temps_jeu, :platine, :commentaire, :statut)";
      } else {
        // Mise à jour d'un jeux_videos existant
        $requete = "UPDATE jeux_videos SET 
                  image_url = :image_url, 
                  titre = :titre, 
                  score = :score, 
      ...
      $stmt->bindParam(':temps_jeu', $temps_jeu);
      $stmt->bindParam(':platine', $platine);
      ...
      if ($stmt->execute()) {
        $message = '
        <div class="alert alert-success alert-dismissible fade show" role="alert">
          Le jeux video a été ' . (empty($id) ? 'ajouté' : 'mis à jour') . ' avec succès
          <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
      } else {
        $message = '
  ...
  // Récupération des jeux video depuis la BDD
  $requete = "SELECT * FROM jeux_videos $where_clause ORDER BY titre ASC";
  $stmt = $dbh->prepare($requete);
  $stmt->execute();
  ...
  <div class="main-content">
    ...
    <div class="mt-4">
      <div class="justify-content-center row">
        <div class="col-md-11">
          <img src="../Capture d'écran 2024-01-10 201320.png" class="img-fluid w-100" alt="MyGameList">
        </div>
      </div>
    </div>
          ...
          <div class="filter-tabs">
            <div class="container">
              <ul class="nav nav-pills">
                <li class="nav-item">
                  <a class="nav-link <?= $filter == 'all' ? 'active' : '' ?>" href="index.php?filter=all">All jeux video</a>
                </li>
                <li class="nav-item">
                    ...
                    <td>
                      <?php if (!empty($film['image_url'])): ?>
                        <img src="<?= htmlspecialchars($film['image_url']) ?>" alt="<?= htmlspecialchars($film['titre']) ?>"
                          class="game-img">
                      <?php else: ?>
                        <img src="placeholder.jpg" alt="No image" class="game-img">
                      <?php endif; ?>
                    </td>
                    <td>
