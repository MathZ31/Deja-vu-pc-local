...
<body>
  ...
  // Traitement de la suppression d'un anime
  if (isset($_GET["action"]) && $_GET["action"] == "delete" && isset($_GET["id"])) {
    $requete = "DELETE FROM animes WHERE id = :id";
  ...
  // Traitement de l'ajout ou de la modification d'un anime
  if (isset($_POST['submit_anime'])) {
    ...
    if (empty($errors)) {
      ...
      if ($stmt->execute()) {
        $message = '
        <div class="alert alert-success alert-dismissible fade show" role="alert">
          L\'anime a été ' . (empty($id) ? 'ajouté' : 'mis à jour') . ' avec succès
          <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>';
      } else {
        $message = '
  ...
  // Récupération des animes depuis la BDD
  $requete = "SELECT * FROM animes $where_clause ORDER BY titre ASC";
  $stmt = $dbh->prepare($requete);
  $stmt->execute();
  ...
  <!-- Sidebar -->
  <div class="sidebar">
    <div class="sidebar-item" data-bs-toggle="modal" data-bs-target="#addAnimeModal">
  ...
  <div class="main-content">
    ...
    <div class="mt-4">
      <div class="justify-content-center row">
        <div class="col-md-11">
          <img src="../Capture d'écran 2024-01-10 201320.png" class="img-fluid w-100" alt="MyAnimeList">
        </div>
      </div>
    </div>
          ...
          <div class="filter-tabs">
            <div class="container">
              <ul class="nav nav-pills">
                <li class="nav-item">
                  <a class="nav-link <?= $filter == 'all' ? 'active' : '' ?>" href="index.php?filter=all">All Anime</a>
                </li>
                <li class="nav-item">
        ...
        <div class="col-md-11">
          <table class="table table-striped table-hover">
            ...
            <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <?php if (!empty($anime['image_url'])): ?>
                        <img src="<?= htmlspecialchars($anime['image_url']) ?>" alt="<?= htmlspecialchars($anime['titre']) ?>"
                          class="anime-img">
                      <?php else: ?>
                        <img src="placeholder.jpg" alt="No image" class="anime-img">
                      <?php endif; ?>
                    </td>
                    <td>
                      <strong><?= htmlspecialchars($anime['titre']) ?></strong>
                      <br>
                      <small class="text-muted">Add notes</small>
                    </td>
                    <td class="text-center">
                      <span
                        class="text-rating-<?= $anime['score'] ? $anime['score'] : '0' ?>"><?= $anime['score'] ? $anime['score'] : '-' ?></span>
                    </td>
                    <td>TV</td>
                    <td><?= $anime['avancement'] ?>     <?= $anime['avancement'] == 1 ? 'épisode' : 'épisodes' ?></td>
                    <td>
                      <button class="btn btn-sm btn-warning edit-anime" data-id="<?= $anime['id'] ?>">
                        <i class="fa-solid fa-pen-to-square"></i>
                      </button>
                      <a href="index.php?action=delete&id=<?= $anime['id'] ?>&filter=<?= $filter ?>"
                        class="btn btn-sm btn-danger"
                        onclick="return confirm('Voulez-vous vraiment supprimer cet anime?');">
              ...
              <?php else: ?>
                <tr>
                  <td colspan="7" class="text-center">Aucun anime dans votre liste pour le moment.</td>
                </tr>
              <?php endif; ?>
            </tbody>
          </table>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="animeModalLabel">Ajouter un anime</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form id="animeForm" method="post" action="">
            <input type="hidden" name="id" id="anime_id">
            ...
            <div class="text-end">
              ...
              <button type="submit" name="submit_anime" class="btn btn-primary">Enregistrer</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <!-- Champ Statut -->
            ...
            <div class="text-end">
              <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
              <button type="submit" name="submit_anime" class="btn btn-primary">Ajouter</button>
            </div>
          </form>
        </div>
      </div>
  ...
  <script>
    // Gestion de la modification d'un anime
    document.addEventListener('DOMContentLoaded', function () {
      // Récupérer tous les boutons d'édition
      const editButtons = document.querySelectorAll('.edit-anime');
      ...
      editButtons.forEach(button => {
        button.addEventListener('click', function () {
          ...
          // Récupérer les données de l'anime via AJAX
          fetch('get_anime.php?id=' + animeId)
            .then(response => response.json())
            .then(data => {
              // Remplir le formulaire avec les données
              document.getElementById('anime_id').value = data.id;
              ...
              document.getElementById('animeModalLabel').textContent = 'Modifier l\'anime';
